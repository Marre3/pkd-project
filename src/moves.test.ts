...
import {
    Color, Move, get_legal_moves, is_check, is_self_check,
    position_from_fen, get_piece_by_square, other_color,
    apply_move_by_notation, get_default_board, make_coordinates,
    is_square_attacked_by, Piece, apply_move, get_move_by_notation
} from "everything";
...
test("is_square_attacked_by_test", () => {
    ...
    expect(
        is_square_attacked_by(board, make_coordinates(4, 3), Color.White)
    ).toBe(true)
    expect(
        is_square_attacked_by(board, make_coordinates(7, 4), Color.White)
    ).toBe(true)
    expect(
        is_square_attacked_by(board, make_coordinates(6, 4), Color.White)
    ).toBe(false)
    expect(
        is_square_attacked_by(board, make_coordinates(5, 4), Color.White)
    ).toBe(true)
    expect(
        is_square_attacked_by(board, make_coordinates(6, 5), Color.Black)
    ).toBe(true)
    expect(
        is_square_attacked_by(board, make_coordinates(5, 5), Color.Black)
    ).toBe(false)
    expect(
        is_square_attacked_by(board, make_coordinates(4, 5), Color.Black)
    ).toBe(true)
})
...
test("is_square_attacked_by_despite_pin_test", () => {
    ...
    expect(
        is_square_attacked_by(board, make_coordinates(6, 1), Color.Black)
    ).toBe(true)
})
...
test("apply_castling_kingside_white", () => {
    ...
    expect(
        get_piece_by_square(king_destination, result_position)
    ).not.toBeNull()
    expect(
        get_piece_by_square(king_destination, result_position)!.piece
    ).toBe(Piece.King)

    const rook_destination = make_coordinates(6, 1)  // f1
    expect(
        get_piece_by_square(rook_destination, result_position)
    ).not.toBeNull()
    expect(
        get_piece_by_square(rook_destination, result_position)!.piece
    ).toBe(Piece.Rook)
...
test("apply_castling_kingside_black", () => {
    const board = position_from_fen(
        "r1bqk2r/pppp1ppp/2n2n2/1Bb1p3/4P3/2P2N2/PP1P1PPP/RNBQ1RK1 b kq - 0 5"
    )
    const move = get_move_by_notation(board, "O-O")
    expect(move).not.toBeNull()
    const result_position = apply_move(board, move!)
...
test("apply_castling_queenside_black", () => {
    const board = position_from_fen(
        "r3k2r/ppp2ppp/2p5/2b5/4P1bq/3P4/PPP2PPP/RNB1QRK1 b kq - 1 9"
    )
    const move = get_move_by_notation(board, "O-O-O")
    expect(move).not.toBeNull()
    const result_position = apply_move(board, move!)
